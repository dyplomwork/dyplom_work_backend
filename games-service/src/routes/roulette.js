...
router.post('/game/play', requireAuth, async (req, res) => {
  ...
  // Validate each bet: amount must be a positive finite number, key must be valid
  for (const b of bets) {
  ...
  for (const { key, amount } of bets) {
    const multiplier = resolveBet(key, number);
    totalReturn += Number(amount) * multiplier;
  }

  if (totalReturn > 0) await addBalance(req.user.id, totalReturn);
  recordStats(req.user.id, 'roulette', totalBet, totalReturn);

  if (totalBet > 0 && totalReturn >= totalBet * 20) {
